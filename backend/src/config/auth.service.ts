...
export async function loginUser(username: string, password: string) {
  ...
  // This overwrites any existing session, instantly logging out
  // any other browser or device that was previously logged in.
  ...
  const payload: JwtPayload = {
    ...
    sessionToken,          // embedded in JWT — verified on every request
  };

  logger.info(`User ${username} logged in — session started (previous sessions invalidated)`);
...
export async function changePassword(
  ...
  await query(
    `UPDATE nps.system_users
     SET password_hash = $1, must_change_pwd = FALSE, password_changed_at = NOW()
     WHERE id = $2`,
    [hash, userId]
  );
}

// Called on logout — clears the session token so the JWT becomes invalid
// even if it hasn't expired yet
export async function logoutUser(userId: string) {
  await query(
    `UPDATE nps.system_users SET session_token = NULL WHERE id = $1`,
    [userId]
  );
}
